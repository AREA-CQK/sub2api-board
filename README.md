...
| **额度追踪** | 5 小时、7 天、30 天或 API Key 配额利用率及重置时间；支持按需查询 OpenAI OAuth 账号的可用重置次数与到期时间 |
| **用量趋势** | 请求、Token、费用切换，支持悬停查看单日数据 |
| **账号展示** | 最多选择 6 个账号，优先展示未用尽且使用率较高的账号 |
| **AI 性能测速** | 并行测试当前账号的首 Token 延迟、生成耗时、输出 Token 和 Token/s，兼容旧版 Sub2API |
| **桌面 Widget** | 小号和中号每次展示 1 个账号，大号展示 2 个账号，每 3 秒轮换，支持组件内手动刷新及重置次数展示 |
| **刷新与恢复** | 支持 15 秒起的自动刷新；Widget 展示刷新结果，失败时回退到最近一次成功快照 |
...
3. 在设置中选择 Widget 要展示的账号和 App/Widget 自动刷新间隔，可选 `15、30、45` 秒，以及 `1、5、10、15、20…60` 分钟。
...
宿主 App 顶部的测速按钮会向当前看板中的每个账号发起一次轻量 AI 请求。新版后端可返回精确输出 Token；连接旧版后端时，App 会基于流式响应估算 Token/s，并以 `≈` 标记估算结果。
